...
public class PayloadTests extends FlowWebhookTests {
	...
	public void setUp()  {
		...
		try {
			mockServer.expect(times(1), requestTo(containsString("flow/execute/5d1a188af6ca2c00014c4314")))
	...
	public void testPayload() {
		RequestFlowExecution requestFlowExecution = new RequestFlowExecution();
		requestFlowExecution.setToken(token);
		
		FlowWebhookResponse flowActivity = webhookController.submitWebhookEvent(requestFlowExecution);
		
		assertNotNull(flowActivity);
		assertNotNull(flowActivity.getActivityId());
	}
